...
IMAGE_EXTENSION = Constants.IMAGE_EXTENSION.value
...
class GetDataFromXMLfile:
    ...
    def parse_xmlfile(self):
        ...
        if xmltree.findall('object') == []: 
            ...
            # In this case, pascal voc has not <object> feild
...
class CreateYOLOfile:
    def __init__(self, xmlfile_datalists_list, classes_list, 
        ...
        try:
            ...
            if IMAGE_EXTENSION not in self.img_filename.lower():
                self.img_filename = self.img_filename+IMAGE_EXTENSION
            (self.img_height, self.img_width, _) = cv2.imread(os.path.join(self.abs_path_img , self.img_filename)).shape
...
def main(u_id, absolutepath_of_directory_with_xmlfiles, 
        absolutepath_of_directory_with_imgfiles, 
        config_path=settings.voc_to_yolo_yaml):
    ...
    absolutepath_of_directory_with_yolofiles = os.path.join(*slice_config['labels_directory']).replace('replace', rand_str)
    make_directories([absolutepath_of_directory_with_yolofiles])

    absolutepath_of_directory_with_classes_txt = os.path.join(*slice_config['classes_path']).replace('replace', rand_str)
    absolutepath_of_directory_with_error_txt = os.path.join(*slice_config['err_file_path']).replace('replace', rand_str)
    ...
    if len(xmlfiles_pathlist) == 0:
        ...
        return [False, 'No XML files could be located']
    ...
    return [True, absolutepath_of_directory_with_yolofiles]
...
if __name__ == '__main__':
    ...
    main(u_id,absolutepath_of_directory_with_xmlfiles, 
            absolutepath_of_directory_with_imgfiles)
